...
class AudioMaster {
    ...
    func playWaypointSound() {
        let waypointSoundSource = createSoundSource("bell", atPosition: audioEnvironment.listenerPosition, volume: 3, options: .interruptsAtLoop)
        waypointSoundSource.play()
    }
    
    func playEndSound() {
        let reachSoundSource = createSoundSource("bell", atPosition: audioEnvironment.listenerPosition, volume: 3, options: .loops)
    ...
    func playSpatialSound() {
        ...
        destinationSoundSource = createSoundSource("drumloop", atPosition: AVAudio3DPoint(x: Float(destinationLocation!.latitude), y: 0, z: Float(destinationLocation!.longitude)), volume: 5, options: .loops)
